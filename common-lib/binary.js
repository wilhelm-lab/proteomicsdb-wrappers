...
var getClosestValues_spec2_FACTORY = function (selection) {
  return function (a, x) {
     ...
     * ppm part
     */
    var f = getClosestValues_specF(selection);
    return	f(a, x)
...
var selectMostIntensePeak = function (mergedSpectrum) {
  ...
  return mergedSpectrum.map((peakObj) => {
    if (peakObj.id_1.length === 0) {
      ...
      mostIntense = peakObj.id_1.map((id, i) => ({
        mz: peakObj.mz_1[i],
...
var compare_ppm_FACTORY = function (property) {
  return function (peak1, peak2, ppm) {
    // asking if peak2 is close to peak1
    var error = 1 / Math.pow(10, 6) * ppm * peak1[property];
 ...
 * value e.g. 10
 */
var compare_FACTORY = function (property, type="ppm", value=10) {
  return function (peak1, peak2) {
    // asking if peak2 is close to peak1
    var error;
    switch(type) {
...
var generate_searchF = function (spectrum, type="ppm", value=10) {
  return function (peak) {
    var getClosestValues_spec2F = getClosestValues_spec2_FACTORY('mz_2');
    var a = getClosestValues_spec2F(spectrum, peak.mz);
    // compare_ppmF = compare_ppm_FACTORY('mz_2');
    var compare_F = compare_FACTORY('mz_2', type, value);
    var is_inside = compare_F(a, peak); // TODO correct here?
    if (is_inside) {
 ...
 */
var binary_search_spectrum = function (spectrum_1, spectrum_2, type="ppm", value=10) {
...
var binary_full_merge = function (spectrum_1, spectrum_2) {
  var merge1 = binary_search_spectrum(spectrum_1, spectrum_2);
  var merge2 = binary_search_spectrum(spectrum_2, spectrum_1);
  var to_add_1 = merge2.intensity_1.filter((x) => x === 0);
  var to_add_2 = merge2.intensity_2.filter((x, i) => merge2.intensity_1[i] === 0);
