...
var Annotation = class Annotation {
  ...
  annotatePeaks(){
    ...
    this.response["fragments"].map((el) =>{ // el are calculated frags
  ...
  calculateFragments(sequence, precursorCharge, mods) {
    ...
    for (var pc = 1; c <= precursorCharge; c++){
      element = {};
      element["charge"] = pc;
      element["isPrecursor"] = true;
      element["type"] = "M";
      element["mz"] = ( this.response["precursorMass"] + c * this.ChemistryConstants["Proton"] ) / pc;
      element["number"] = "+" + pc + "H";
      element["neutralLoss"] = "";
      fragments.push(element);
    }
